...
class Ui_MainWindow(QWidget):
    ...
    def show_hist(self):
        old_img = cv2.imread(self.textBrowser.text(), 1)
        path = self.new_img_name(self.textBrowser.text())
        new_img = cv2.imread(path[0], 1)

        (par_old, pixel_old, c_old) = plt.hist(old_img.ravel(), 256)

        (par_new, pixel_new, c_new) = plt.hist(new_img.ravel(), 256)
        fig, (img1, img2) = plt.subplots(1, 2)
        fig.suptitle('Histograme ')
        img1.set_title("old image")
        img2.set_title("new image")
        img1.plot(par_old)
        img2.plot(par_new)
    ...
    def getText(self):
        new_path = self.new_img_name(self.textBrowser.text())

        api = ocrspace.API('71682b402388957')
        self.text_label.setText(api.ocr_file(new_path[0]))

    def Save(self):  # replace it  save hist
        new_path = self.new_img_name(self.textBrowser.text())
        plt.savefig(new_path[1])

    def Enhance(self):

        new_path = self.new_img_name(self.textBrowser.text())
        nimag = self.enhance_image(self.textBrowser.text(), new_path[0], write=1)
        self.out_img.setPixmap(QPixmap(new_path[0]))

        ########## help functions ############

    def new_img_name(self, path):
        k = 0
        ...
        return (path[:k + 1] + "new_" + path[k + 1:]), (path[:k + 1] + "hist_" + path[k + 1:])
